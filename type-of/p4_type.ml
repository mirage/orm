...
let gen tds =
  ...
  let subst_external_var (_loc, name, t) =
    ...
    | Var v                             -> <:expr< Orm.Type.Var $str:v$ >>
    | Rec (v, t)                        -> <:expr< Orm.Type.Rec ($str:v$, $aux t$) >>
    | Unit                              -> <:expr< Orm.Type.Unit >>
    | Int                               -> <:expr< Orm.Type.Int >>
    | Int32                             -> <:expr< Orm.Type.Int32 >>
    | Int64                             -> <:expr< Orm.Type.Int64 >>
    | Float                             -> <:expr< Orm.Type.Float >>
    | Bool                              -> <:expr< Orm.Type.Bool >>
    | Char                              -> <:expr< Orm.Type.Char >>
    | String                            -> <:expr< Orm.Type.String >>
    | Option t                          -> <:expr< Orm.Type.Option $aux t$ >>
    | Product tl                        -> <:expr< Orm.Type.Product $List.fold_left (fun accu x -> <:expr< [ $aux x$ :: $accu$ ] >>) <:expr< [] >> tl$ >>
    | Collection t                      -> <:expr< Orm.Type.Collection $aux t$ >>
    | Named_sum ts                      -> 
      let rec fn accu = function
      | []          -> accu
      | (n, t) :: l -> <:expr< [ ( $str:n$, $List.fold_left (fun accu x -> <:expr< [ $aux x$ :: $accu$ ] >>) <:expr< [] >> t$ ) :: $fn accu l$ ] >> in
      <:expr< Orm.Type.Named_sum $fn <:expr< [] >> ts$ >>
    | Named_product ts                  ->
      let rec fn accu = function
      | []              -> accu
      | (n, M, t) :: l -> <:expr< [ ($str:n$, Orm.Type.M, $aux t$) :: $fn accu l$ ] >>
      | (n, I, t) :: l -> <:expr< [ ($str:n$, Orm.Type.I, $aux t$) :: $fn accu l$ ] >> in
      <:expr< Orm.Type.Named_product $fn <:expr< [] >> ts$ >>
    in
