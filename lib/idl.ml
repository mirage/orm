...
module Field = struct
  ...
  let add_foreign ~env ~t_name ~f_name =
    assert_field_not_exist ~env ~t_name ~f_name;
  ...
  let foreigns env t : field list = filter_fields_with_table (is_foreign env) env t
  ...
  let get_data_type env f =
    match f.f_info with
    | Data d -> d
    | _      -> error env "%s is not a data field" f.f_name
  ...
  let exposed env = filter_fields_with_table (is_exposed env) env
...
module Table = struct
  ...
  let add ~env ~t_name ~t_info ~t_exposed ~parent =
    ...
    let table = {
      ...
      t_parent  = [];
      t_exposed = t_exposed;
      t_loc     = Env.loc_of_table env t_name;
    } in
...
let rec process_sig ~env ~t_name ~t_exposed ~name s =
  ...
  | Product sl ->
    ...
    let env, _ = List.fold_left (fun (env, pos) t ->
      let elt_name = sprintf "%s__%d" name pos in
      let env = process_sig ~env ~t_name:name ~t_exposed:false ~name:elt_name t in
      let elt_table = Table.find env elt_name in
      Field.add ~env ~t_name:name ~f_name:elt_name ~f_info:(Foreign (Internal, elt_table.t_name)), pos+1
      ) (env, 0) sl in
    env
  ...
  | Named_product sl ->
    ...
    List.fold_left (fun env (f_name, is_mutable, s) ->
      ...
      let env = process_sig ~env ~t_name:name ~t_exposed:false ~name:field_name s in
      let field_table = Table.find env field_name in
      Field.add ~env ~t_name:name ~f_name:field_name ~f_info:(Foreign (Internal,field_table.t_name))
      ) env sl
  ...
  | Named_sum sl ->
    ...
    List.fold_left (fun env (r_name, sl) ->
      let env, _ = List.fold_left (fun (env, pos) s ->
        let row_name = sprintf "%s__%s__%i" name r_name pos in
        let env = process_sig ~env ~t_name:name ~t_exposed:false ~name:row_name s in
        let row_table = Table.find env row_name in
        Field.add ~env ~t_name:name ~f_name:row_name ~f_info:(Foreign (Internal, row_table.t_name)), pos+1
        ) (env, 0) sl in env
      ) env sl
  ...
  | Var v      ->
    ...
    Field.add_foreign ~env ~f_name:name ~t_name:v

let process env =
  let env = List.fold_left
    (fun env (_, name, s) -> process_sig ~env ~t_name:"__toplevel__" ~t_exposed:true ~name s)
    env env.e_type in
